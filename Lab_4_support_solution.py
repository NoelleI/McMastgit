...
print("However, we do not know what the p-values or error on these coefficents are, interpret with caution")
print("Average Revenue", df_final["revenue"].mean())
...
#See what a regression looks like
...
lin_score_test_norm = lin.score(X_test, y_test)
lin_score_train_norm = lin.score(X_train, y_train)

print("Training score with norm " + str(lin_score_train_norm) + " training score without norm " + str(lin_score_train_drop))
print("Testing score with norm: " + str(lin_score_test_norm) + " testing score without norm " + str(lin_score_test_drop))

print("Coefficients",lin.coef_)  #coef are around the size of the ratio of revenue to 1 (magnitude of dummy vars), but signs should be investigated 
print("For predictors " + str(X_test.columns))
print("Intercept",lin.intercept_)

#Do RFECV: recursive feature elimination
#lr = linear_model.LinearRegression()
lr = linear_model.Ridge()

from sklearn.feature_selection import RFECV

rfe = RFECV(lr, step = 1, scoring = 'r2', cv = 10)
#rfe does the splittimg, so merge train and test sets for the input
...
print("the r-squared is " + str(rfe.score(X,y)))
print("the feature rankings are " + str(rfe.ranking_))
print("the support is " + str(rfe.support_))
print("the grid scores are " + str(rfe.grid_scores_))
print("the maximum grid score was at step " + str(np.argmax(rfe.grid_scores_)))
print("running this several times shows instability. This dataset is sparse and highly multicollinear. More advanced methods are required")

#this looks unstable the values of the coefficients are unreliable due to multicolinearity, sparsity, high dimensionality of dataset
#also, errors are reported when too many countries/genres/companies are grouped together - i.e. when only the most frequent appear
#Here are some outputs for several runs in a row:
